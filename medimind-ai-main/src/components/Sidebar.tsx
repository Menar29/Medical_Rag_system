    ...
    <aside className="hidden md:flex w-72 shrink-0 flex-col border-r border-border/60 bg-sidebar/70 backdrop-blur-xl">
      ...
      <div className="flex items-center gap-3 px-4 h-16 border-b border-border/50">
        <div className="relative">
          <div className="h-9 w-9 rounded-xl bg-gradient-to-br from-medical to-violet-soft grid place-items-center shadow-elegant">
            <Stethoscope className="h-4.5 w-4.5 text-primary-foreground" strokeWidth={2.4} />
          </div>
      ...
      <div className="p-3">
        ...
        >
          ...
          <kbd className="hidden lg:inline text-[10px] text-muted-foreground border border-border/70 rounded px-1.5 py-0.5">
            ⌘N
          </kbd>
        </Link>
      </div>
      ...
      {/* History */}
      <div className="px-4 pt-3 pb-1 text-[11px] uppercase tracking-wider text-muted-foreground/80">
        {t("history")}
      </div>
      <div className="flex-1 overflow-y-auto px-2 pb-3 space-y-0.5">
        <AnimatePresence initial={false}>
          {conversations.length === 0 && (
            <div className="px-3 py-6 text-xs text-muted-foreground/70 italic">
              —
            </div>
          )}
          {conversations.map((c) => {
