...
> {
  // Real implementation (left commented for backend wiring):
  ...
  // while (true) { const { value, done } = await reader.read(); if (done) break; yield { type: "chunk", text: decoder.decode(value) }; }
